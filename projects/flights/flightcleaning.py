...
for csv_file_name in csv_file_list:
    chunk_container = pd.read_csv(csv_file_name, chunksize=csize,
                                  skiprows=[0], usecols=cols_to_keep,
    ...
    for chunk in chunk_container:
        ...
        # delays should be int
        chunk.iloc[:, -5:] = chunk.iloc[:, -5:].astype('int')
        # process row id
        n_chunk = len(chunk)
        chunk.index = np.arange(num_rows, num_rows+n_chunk, dtype=np.int32)
        num_rows += n_chunk
